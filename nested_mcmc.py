...
for i in range(steps):
    ...
    keep.append([live_points[worst]])
    keep[i, :2] = live_points[worst]
    keep[i, 2:6] = logL[i], logX[i+1], logw[i], logZ[i+1]
    

    for j in range(mcmc_iter):
        
        new = proposal(live_points[worst])
    ...
    print('time taken per iteration: %f seconds' %((t2-t1)))
    print('time elapsed: %f seconds'%((t2-start)))
          
file = h5py.File('samples_data_2.hdf5', 'w')
